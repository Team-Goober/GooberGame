...
namespace Sprint.Factory.Door
{
    internal class DoorFactory
    {
        ...
        /// <returns></returns>
        public IDoor MakeDoor(string type, string spriteFile, string spriteLabel, Vector2 position, Vector2 size, Vector2 openSize, int otherSide)
        {
            ...
            {
                case "open":
                    return new OpenDoor(sprite, position, size, openSize, otherSide, objManager);
                case "wall":
                    return new WallDoor(sprite, position, size, openSize, otherSide, objManager);
                case "lock":
                    return new LockDoor(sprite, position, size, openSize, otherSide, objManager);
                case "hidden":
                    return new HiddenDoor(sprite, position, size, openSize, otherSide, objManager);
                default:
                    break;
            }
